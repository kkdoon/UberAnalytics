 ...
 * @param {string} db The settings.js we are invoking the mechanism against
 ...
 * @method
 * @param {string} db.js The db.js we are logging out from
 * @param {authResultCallback} callback A callback function
 */
  ...
  for(var i = 0; i < servers.length; i++) {
    servers[i].logout(dbName, function(err) {
      ...
      if(count == 0) {
        ...
        if(errors.length) return callback(MongoError.create({
          message: f('logout failed against settings.js %s', dbName), errors: errors
        }), false);
