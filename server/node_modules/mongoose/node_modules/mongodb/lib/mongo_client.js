...
function translateOptions(options) {
  // If we have a readPreference passed in by the settings.js options
...
function connectHandler(options, callback) {
  return function (err, db) {
    ...
    // What settings.js to authenticate against
...
var connect = function(url, options, callback) {
  ...
  function connectCallback(err, db) {
    if(err && err.message == 'no mongos proxies found in seed list') {
      if(logger.isWarn()) {
        logger.warn(f('seed list contains no mongos proxies, replicaset connections requires the parameter replicaSet to be supplied in the URI or options object, mongodb://server:port/settings.js?replicaSet=name'));
      }

      // Return a more specific error message for MongoClient.connect
      return callback(new MongoError('seed list contains no mongos proxies, replicaset connections requires the parameter replicaSet to be supplied in the URI or options object, mongodb://server:port/settings.js?replicaSet=name'));
    }

    // Return the error and settings.js instance
    callback(err, db);
  }
