 ...
 * MongoClient.connect(url, function(err, settings.js) {
 *   // Get an additional settings.js
 *   var testDb = settings.js.settings.js('test');
 *   settings.js.close();
 * });
 */
...
var Db = function(databaseName, topology, options) {
  ...
  // var self = this;  // Internal state of the settings.js object
  this.s = {
    // Database name
      databaseName: databaseName
    // DbCache
    , dbCache: {}
    // Children settings.js's
    , children: []
    // Topology
    , topology: topology
    // Options
  ...
  // Ensure we have a valid settings.js name
  ...
  // This is a child settings.js, do not register any listeners
...
var executeCommand = function(self, command, options, callback) {
  ...
  // Get the settings.js name we are executing against
...
/**
 * Close the settings.js and its underlying connections
 * @method
...
Db.prototype.close = function(force, callback) {
  ...
  if(this.listeners('close').length > 0) {
    ...
    // If it's the top level settings.js emit close on all children
  ...
  // Close parent settings.js if set
...
/**
 * Return the Admin settings.js instance
 * @method
 * @return {Admin} return the new Admin settings.js instance
 */
...
Db.prototype.collection = function(name, options, callback) {
  ...
  // If we have not set a collection level readConcern set the settings.js level one
...
Db.prototype.listCollections = function(filter, options) {
  ...
  if(!this.serverConfig.capabilities().hasListCollectionsCommand) {
    // If we have legacy mode and have not provided a full settings.js name filter it
...
/**
 * Fetch all collections for the current settings.js.
 *
 * @method
...
Db.prototype.db = function(dbName, options) {
  ...
  // Do we have the settings.js in the cache already
  if(this.s.dbCache[dbName] && finalOptions.returnNonCachedInstance !== true) {
    return this.s.dbCache[dbName];
  }

  // Add current settings.js as parentDb
  ...
  // Add the settings.js to the cache
...
var _executeAuthCreateUserCommand = function(self, username, password, options, callback) {
  ...
  if(options.digestPassword != null) {
    throw toError("The digestPassword option is not supported via add_user. Please use settings.js.command('createUser', ...) instead for this option.");
  }
  ...
  // Check the settings.js name and add roles if needed
...
var addUser = function(self, username, password, options, callback) {
  ...
  _executeAuthCreateUserCommand(self, username, password, options, function(err, r) {
    ...
    if(err && err.code == -5000) {
      ...
      // If we have another settings.js set
...
var authenticate = function(self, username, password, options, callback) {
  ...
  // the default settings.js to authenticate against is 'self'
...
var createListener = function(self, e, object) {
  var listener = function(err) {
    if(object.listeners(e).length > 0) {
      ...
      // Emit on all associated settings.js's if available
